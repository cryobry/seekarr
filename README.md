...
Set the top-level `interval` to the number of seconds between runs, or `0` to run once and exit. It can be overridden with `APP_INTERVAL` or `--interval`.
...
| `--interval SECONDS` | Override `interval` and loop forever; `0` runs once | `APP_INTERVAL`, then `config.yml`, then `300` in Docker or one run locally |
